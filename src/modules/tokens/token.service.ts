...
export class TokenService {
    ...
    async generateTokens(payload: ITokenPayload) {
        ...
        // 1. Приведение типа:
        // Мы явно объявляем payload как простой object, чтобы избежать
        // конфликта с индексированным типом ITokenPayload.
        
        // Если этого недостаточно, используем явное приведение:
        // const jwtPayloadObject = payload as object; 
        console.log('this.config.get<string>("JWT_SECRET")',this.config.get<string>("JWT_SECRET"))
        // 2. Генерация токенов с использованием правильного типа (object)
        const tokens = {
            access_token: await this.jwtService.signAsync(payload, {
                secret: this.config.get<string>("JWT_SECRET"),
                expiresIn:'1d'
                // Теперь TypeScript должен корректно видеть 'expiresIn',
                // так как он выбрал 3-й оверлод (payload: object)
                // expiresIn: this.config.get<string>("ACCESS_EXPIRED"), 
            }),
            refresh_token: await this.jwtService.signAsync(payload, {
                secret: this.config.get<string>("JWT_REFRESH_SECRET"),
                expiresIn: this.config.get<number>("REFRESH_EXPIRED") 
            })
        }
